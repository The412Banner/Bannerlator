...
## 2026-08-27 — win-fg teardown reset SHIPPED + lsfg Fold-8/a840 diagnosis (RESUME point)
**win-fg = DONE, merged to main `68b528d9`** (vc 75, no release): full lsfg-style pause+surface-teardown+Resume reset on EVERY win-fg change (On/Off/multiplier/model/preset; Flow Scale live) via `maybeTriggerWinFgReset`; bundled win-fg `.so` = pacing-OFF `0096525b` (headroom flicker fix + a840 present-id freeze fix + no judder + swapchain-recreate conf-reload so a preset change teardown doesn't collide+freeze on the 840). Two merges: `717fe830` (toggle teardown + pacing-off, Fold-proven for modes) then `68b528d9` (preset teardown + .so fix). ⚠️ preset-teardown fix was a BLIND build (AYANEO/750 can't repro the 840 freeze); revert `git revert -m 1 68b528d9` if it regresses on the Fold.

**lsfg NOT working on Fold-8/a840 — ROOT CAUSE FOUND: lsfg never armed at launch.** Fold diagnostic log (game DOOMBLADE, standard flavor com.winlator.banner) proved NO `ENABLE_LSFG` in the guest env, zero `lsfg-vk-framegen` lines → `resolvedFrameGenEngine()` returned "off" at launch → layer never loaded (so "no fps/no difference" and the debug-dump made no folder). lsfg arms ONLY at launch via `ENABLE_LSFG`; the in-game drawer can't load a layer not armed at startup. User: SAME game works on the AYANEO/750 → likely the Fold's shortcut/container engine=off at launch (config or a persist/read bug). Also found 2 real bugs: `presentWaitDevice=false` misdetection on a real a840 that advertises `VK_KHR_present_wait` (host-side ext probe empty for custom adrenotools drivers → model-match fallback failed), and a stray `libVkLayer_LSFGVK_frame_generation.so` in APK jniLibs failing host-process load.

**Diag2 build STAGED for the Fold (NOT merged, Fold-test-first):** branch `lsfg-diag-n1n2` `f757dade`, CI `33117458456` GREEN. Adds a launch-arming `Log.i` (`frame-gen setup: engine=… renderer=… Lossless.dll=… presentWaitDevice=…` + `lsfg ARMED`/`NOT armed`), the debug_dump toggle (Settings → Download/lsfg-dump), N1 (skip lsfg teardown on present_wait devices), N2 (real host mailbox while generating), the present_wait-detect fix, and drops the stray VkLayer stub. Standard-flavor APK for the Fold: `/sdcard/Download/Bannerlator-lsfg-diag2-standard.apk` (sha `d92e1436`); pubg staged too (`c3e1500f`). **RESUME:** user installs the standard diag2 on the Fold → sets engine=lsfg in DOOMBLADE's PRE-LAUNCH shortcut + SAVE (not the in-game drawer) → launches fresh → Start diagnostic log → sends `Download/win-fg-logs/*.log`; the new launch line shows definitively why lsfg armed or didn't. Full checkpoint at the top of memory `project_bannerlator_bionic_fg_011_upgrade`. AYANEO state: installed `1ad7313a` (winfg-preset-teardown pubg), prefix win-fg .so `9ecedfc7` pacing-off immutable.
