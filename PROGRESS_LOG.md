...
## 🧬 FULL UPSTREAM SYNC MERGE + CI GREEN (2026-08-17)
**What:** Fork = upstream(`1d679738`)-current + VEGAS delta, merged via `cd6f0b5a` (parents `b07f9816` + `1d679738`) after user-approved **full sync (Option A)** — the stale-index tree-rebuild had pruned 298 fork-birth files from HEAD; 294 restored from upstream at merge time. Then serialized CI failures to **all-3-flavor green** (standard / ludashi / pubg, `assemble*Release`).

**Key decisions (with rationale held across the cycles):**
- VEGAS-protected set never overwritten: `container/Vegas*`, `contentdialog/VegasKey*`, `assets/vegas_*.json`, `VegasDownloadSheet.kt`, `ContainerDetailScreen.kt` (hybrid), `ContainerDetailViewModel.kt`, `AppNavGraph.kt`, `AppDrawer.kt`, `Screen.kt`, `ContentDownloadSheet.kt`, `PROGRESS_LOG.md` — extended LATER by `contentdialog/DXVKConfigDialog.java` (VEGAS API carrier; the 142-file sweep `bbaf66e` had accidentally clobbered it with upstream's).
- `ContainerDetailViewModel.kt` → **upstream wholesale** (it carries `EDIT_DEFAULTS_ID`, `defaultsArch`, `defaultsMode` that upstream MainActivity:389 needs; our fork VM had zero VEGAS content — grep-verified).
- `DXVKConfigDialog.java` → **UNION, not wholesale**: keep upstream's D7VK (`D7VK_BUNDLED`/`loadD7vkVersionList` — `XServerDisplayActivity:6211` calls it) + re-add fork API (`StockSource`, `loadVegasStockSources` + provenance sidecar probe, `loadVegasKeyKnowledge`/`loadVegasKeyCatalog`, `VEGAS_KNOWLEDGE_ASSET`/`VEGAS_KEY_CATALOG_ASSET`, `FileUtils` import).
- `ContainerDetailScreen.kt` (ours, hybrid) kept whole under Option A (750+ lines of VEGAS editor UI @ `DxvkConfigDialog` compose section) + surgical ports from upstream: `helpRes`/`HelpDialog` + `showAudioSettings` state into `TopLevelFields` and `GraphicsDriverConfigDialog` (local scopes — TopLevelFields is top-level, can't see the screen's locals), `driverFellBack`/`isCustomDriver` declarations, components imports, `StartupServicesToggleList` composable (ShortcutsScreen:7227 needs it; shared with CDS by design), `LabeledDropdown` D-pad params (`focused`/`expandedOverride`/`onExpandedChange`/`highlightedIndex` — ShortcutsScreen:5328), `DxvkConfigDialog` += `onDownloadD7vk` + `relaxDxvkFilter` + D7VK dropdown/gear row (`ddrawrapper == "d7vk"`) + VKD3D→DXVK 2.x filter (#113) with wrapper-compat-bcn relax (#137) + `d7vkVersion` persist, `border`/`background` imports.
- `InputControlsScreen.kt`: upstream null-check on `importProfile` (`?: throw IllegalArgumentException("Unsupported control profile version")`) — upstream had it, our conflict-resolution copy lost it.

**Commit chain (all pushed to `main`):** `cd6f0b5a` merge-full-sync → `3fd57c5` (toInstallCardState, Screen.Wrappers/BigPicture routes, manifest `BannerlatorApp`) → `f8ddb7a` (values assets) → `fdb3f18` (drop dead `bionic-fg` submodule — 404'd clone) → `fe20730` (restore jitpack for `libsu:6.0.0`) → `1b68b62` (strings union: fork base + 285 upstream keys) → `bbaf66e` (142 divergent files → upstream; fixed the 788-err provider mismatch) → `64ba9e9` (re-apply strings union after sweep clobbered it) → `cd5b22a` (DXVKConfigDialog union + CDS/CDVM fixes + plurals: `custom_icon_in_use`, `custom_icon_profiles_in_use`, `profiles_not_imported` — the union script only captured `<string>`, not `<plurals>`; → `ba062dd` (StartupServicesToggleList + imports) → `44fbf47` (D-pad LabeledDropdown, D7VK row, relaxDxvkFilter) → `6d57be6` (border/background imports).

**CI runs:** `32055810779` clone-fail(bionic-fg) → `32056217895` libsu → `32057675046` strings AAPT → `32058921556` 788 errs → `32060425361` strings-clobber AAPT → `32061284051` 183 errs (plurals/EDIT_DEFAULTS_ID/CDS hybrids) → `32063804256` (3 classes) → `32066781650` (border/background) → **`32069591098` ✅ GREEN (3/3 flavors)** on `6d57be6`.

**Verification:** 3/3 flavor CI green on the final sha; all 8 VEGAS blobs still byte-identical at origin (VegasActiveConfig `69072bff`, VegasMigration `90be6279`, VegasLiveCheck `baf4e41f`, VegasKeyCatalog `d98bdd77`, VegasKeyKnowledge `c79c3284`, vegas_knowledge.json `16f40686`, vegas_key_catalog.json `b75b5e5d`, VegasDownloadSheet `9000e290`); harnesses still 266/266 (pre-existing).

**Known issues / next steps:**
- [ ] Device compile-verify checklist (the final gate — CI-green ≠ installable/running): install standard APK → containers CRUD, VEGAS sheet, DXVK config two-source editor, audio settings dialog, DirectAudio warning, custom-driver extension picker, D7VK dropdown, controller/D-pad dropdown behavior.
- [ ] `d7vkVersion` config write + read-back sanity (new field; upstream semantics).
- [ ] Push-credential caveat: local git remote credential now presents `isygold1` (403) — pushes require token URL (`x-access-token:${GH_TOKEN}`).

**Known issues (carried):** local partial-clone worktree: `git diff`/whole-tree walks hang — use per-path `ls-files -s`/`rev-parse`; no local Kotlin toolchain — compile only via CI.
