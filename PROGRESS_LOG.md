...
## 2026-07-26 — 🎯 **Gyro mouse "hidden offset" at screen edge (issue #175) — root cause is `CursorLocker`, not the AMA bot's answer** (branch `fix/gyro-cursor-edge-windup`, vc51)

> **Reporter-confirmed on-device:** with the visible desktop cursor (relative-mouse OFF), pushing the gyro cursor into a screen edge and tilting *further* discards the past-edge rotation, so tilting back overshoots **past** the start toward the opposite side. (With relative-mouse ON the cursor is captured — icon changes over UI but no visible sprite movement — so no overshoot there; not this bug.) **Root cause:** `Pointer.x/y` are stored UNCLAMPED (clamped only on read via `getClampedX/Y`), BUT `core/CursorLocker.java` — a 60 Hz TimerTask enabled in exactly the desktop case (`XServer.setRelativeMouseMovement`: `cursorLocker.setEnabled(!relativeMouseMovement)`) — writes a clamped+damped value **back** into the pointer every tick, so the past-edge position is never retained. **The 🤖 AMA bot answer on #175 was WRONG:** it blamed the relative-mode `CURSOR_POS_FEEDBACK` overwrite, declared the desktop path "already works correctly," and **missed `CursorLocker` entirely**; its patch is a no-op for the reported (desktop) case. Posted a public correction on the issue + a note that the AMA bot is a direction-pointer, not a reliable source of exact fixes.
> **Fix (`WinHandler.updateGyroMouse`, desktop branch):** keep an UNCLAMPED virtual pointer (`gyroPointerVirtualX/Y`), seed it from the live clamped cursor on activator-press (`gyroPointerSeeded`), integrate deltas into it bounded to ±1 screen (wind-up cap), and drive the pointer toward its **clamped** target via the locked `injectPointerMoveDelta` path. Displayed pointer stays in-bounds ⇒ CursorLocker never fires ⇒ virtual retains past-edge motion ⇒ returning the device returns the cursor. +25/−1, one file. ⚠️ NOT yet device-verified — this CI build is the proving ground; reporter (Devaspe) to re-run the edge round-trip.
