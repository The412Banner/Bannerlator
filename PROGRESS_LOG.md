...
>
> **✅ Two fixes shipped (build `0d285c13`, CI 29321379836 green, device-installed).** (1) **Host-first launch crash** — hosting before launch brought up our DNS-less scoped VPN as the active network; `GuestProgramLauncherComponent.execGuestProgram():394` did `dnsServers.get(0)` on an empty list → IndexOutOfBounds. Guarded (fall back 8.8.4.4) + `import android.net.LinkProperties`. (2) **Subnet-directed broadcast capture** — LAN games broadcast to the real Wi-Fi directed-bcast (e.g. `192.168.1.255`), not `255.255.255.255`, which our scoped tun didn't route → escaped out Wi-Fi. Now: VpnService `computeUnderlyingDirectedBroadcast()` routes it into the tun + passes to native; `lsw_classify` treats `local_bcast` as BROADCAST; `tunnel.c normalize_broadcast()` rewrites forwarded bcasts src→self vIP, dst→255.255.255.255, fixes IP cksum, zeroes UDP cksum (deliverable + replyable on ANY peer subnet). Standalone `make check` green incl cross-subnet normalize. **Device-proven ADAPTS across networks:** home `192.168.1.255` → 5G hotspot `192.168.12.255` (auto). Peer pairing proven over BOTH networks.
>
> **🔑 P1.6 testing verdict (FlatOut 2 + DiRT 3):** both games' LAN HOSTS are SILENT — discovery is CLIENT-query-driven (tcpdump: tun0 0 pkts, host emits only HTTPS). A single-device host + dumb relay-peer can NEVER see game traffic. **GAME-TRAFFIC OBSERVATION NEEDS A 2ND REAL INSTANCE.** Transport thoroughly proven; live 2-player obs blocked on a 2nd device. (FlatOut 2 "online MP" = OpenSpy/GameSpy over public internet, bypasses overlay. Latent: kernel `local`-table wlan0 directed-bcast can shadow our tun route for directed-bcast games — moot til real traffic.) **🟡 New intermittent host-FIRST black-screen hang** (freezer race, NOT the fixed crash; not yet root-caused — capture caught a success; workaround = host-after-launch).
>
> **✅ Goldberg LAN mode — increment 1 (commit `638e376c`, CI 29331544294 green, APK sha-verified).** One-tap toggle for Goldberg-patched Steam-networking co-op games: writes `custom_broadcasts.txt`=peer overlay vIP (host→10.99.0.2, client→10.99.0.1) into the game's Goldberg `steam_settings` so its Steam-LAN discovery goes as directed UNICAST over the overlay (proven LSW_UNICAST), auto — the mechanism PC users hand-edit for Hamachi. `net/GoldbergLanMode.kt` + `GoldbergPatcher.steamSettingsDirs()` + `SteamPrefs.get/setGoldbergLanEnabled` + `LanMultiplayerDialog` toggle (best-effort copy "for games whose multiplayer uses Steam networking") + `ShortcutsScreen` per-game wiring. **DEVICE-PROVEN (UI):** user Goldberg-patched FlatOut 2 → toggle appeared (absent before) ⇒ eligibility + appId→installDir heuristic work. ⚠️ FlatOut 2 won't benefit (GameSpy, not Steam-net) = live demo of "patched≠works". Best-effort flags: heuristic reverse-map + gbe_fork read-loc TODO (both safe). **NOT yet proven:** file-write half + a real Steam-net game using it.
>
> **📚 User library best Goldberg-LAN fits:** ⭐Portal 2 (2-player co-op matches our 2-peer overlay EXACTLY; Steam-net; light; canonical Goldberg-over-VPN) → TOP; alts Titan Quest AE, Borderlands 2, L4D2. Big Source/GoldSrc collection = direct-connect (`connect 10.99.0.1`) overlay lane. No-go: Brawlhalla/Apex/Dota2/CS2/PUBG/etc (server-based/F2P/SP). **⏸️ PAUSED pending: (a) a Steam-net co-op game downloaded+Goldberg-patched (Portal 2), (b) a 2nd device.** Living report (internal `/sdcard/Download/Bannerlator-LAN-Report.html` + public https://the412banner.github.io/bannerlator-lan/) updated, 0-IP verified. **▶️ Other open lanes (single-device buildable): PH = Noise IK crypto + relay abuse-hardening; host-first hang fix.**
