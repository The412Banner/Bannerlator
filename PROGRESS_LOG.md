...
## 2026-08-05 — 🖼️ **DISPLAYX RENDERER REBASED ONTO CURRENT MAIN** — branch `feat/displayx-renderer-current`
> **Why:** the original `feat/displayx-renderer` (commits `7313af1f` + `a892e9f3`) is ~570 commits behind `main`; user wants the existing DisplayX SurfaceControl host renderer buildable/selectable on TODAY's code so it can be device-measured against our ASurfaceRenderer.
> **What:** new branch off `origin/main` (`f45907a8`); cherry-picked both DisplayX commits. Auto-merge was CLEAN for every code file — the only conflicts were `PROGRESS_LOG.md` (kept both histories) and two comment-only lines + one dropdown widget swap. The per-game renderer dropdown had since been refactored from `LabeledDropdown` to `DpDrop`-in-a-`Row` (with help IconButton); re-applied DisplayX into the current `DpDrop` shape in `ShortcutsScreen.kt`, routing both SurfaceFlinger and DisplayX through `SurfaceFlingerWarningDialog` via a `pendingRenderer`. Container editor (`ContainerDetailScreen.kt`/`ContainerDetailViewModel.kt`) auto-merged; `rendererEntries` already lists DisplayX.
> **Integration verified against current APIs:** `HostRenderer` iface unchanged (all 22 methods impl'd); `WindowManager.OnWindowModificationListener` (create/reparent/map/unmap/destroy/zorder/content/geometry/attributes, all defaults), `CursorManager.OnCursorModificationListener`, `Pointer.OnPointerMotionListener.onPointerMove(short,short)` all match; `GPUImage.getHardwareBufferPtr()` accessor present (native reads it via JNICache); `Drawable.setAsrMode` guarded so DisplayX takes the plain non-ASR Drawable path; CMake `add_library(displayx SHARED …)` self-contained (log/android/dl/atomic, all ASurfaceControl_*/AChoreographer_* dlsym'd → no unguarded-availability, no -Werror in tree); renderer identifier round-trips `DisplayX`↔`displayx` via `StringUtils.parseIdentifier` exactly like SurfaceFlinger.
> **Scope:** host renderer only. NO guest-side Vulkan WSI layer, NO real acquire-fence FDs (`setBuffer(…,-1)` kept), NO RGBA/format negotiation — those remain the documented follow-up for the zero-copy win. NOT merged to main; new branch only. Building via CI (`build-artifacts.yml`) to a green artifact for manual install.
...
## 🖼️ DISPLAYX AS A 4th HOST RENDERER (2026-07-21) — Phase A+B+C landed, NOT compiled/tested yet
**Why:** user asked to add DisplayX and measure it, having heard the case against (our ASurfaceRenderer already implements the same architecture, with fences + a BGRA→RGBA converter DisplayX lacks). Faithful port on purpose — his fence-less `setBuffer(..., -1)` and CPU memcpy are kept as-is so the measurement is of HIS design, not a hybrid. Hardening only if it earns it.

**What DisplayX actually is** (two things share the name): the standalone repo `Pipetto-crypto/DisplayX` is an *unfinished, unlicensed* Vulkan xcb/xlib WSI layer — its app-side server is commented out in his own tree with "I will rewrite this once the infrastructure is finished", and it is NOT what ships. The integrated thing (branch `winlator_bionic`, MIT) is a SurfaceControl compositor: one `ASurfaceControl` per X window backed by an AHardwareBuffer, cursor as its own layer, AChoreographer-driven native thread. That is what was ported.

**Landed on branch `feat/displayx-renderer`:**
- **A — X-server events (shared code).** `WindowManager.OnWindowModificationListener` gains `onCreateWindow`/`onReparentWindow`; new `CursorManager.OnCursorModificationListener` (create/free). ALL defaulted no-op → GL/Vulkan/ASR provably unaffected. DisplayX is the only renderer that mirrors the window tree natively, hence the only consumer. Z-order was NOT plumbed: upstream's `nativeChangeWindowZOrder` is a no-op in DisplayX mode (layering comes from SurfaceControl parentage + creation order).
- **B — native.** New `cpp/displayx/` = his `displayx.{cpp,hpp}` + `window/cursor/drawable/view_transformation` mirror + our own `displayx_jni.{hpp,cpp}`. New `libdisplayx.so` target modelled on `direct_scanout`: log/android/dl/atomic only — NO EGL/GLES/vulkan/adrenotools. Divergences from upstream: no `isDisplayX()` fork (own .so, only loaded when selected); root cursor bitmap passed in from Java (`R.drawable.cursor`) instead of stb_image on assets/cursor.png; `GPUImage.hardwareBufferPtr` read via the `getHardwareBufferPtr()` accessor (ours is private, upstream's is a public field); dead commented-out mock Vulkan-WSI server (upstream lines 800-1038) stripped; `nativeUpdateDirectContent` uses `find()` instead of upstream's `operator[]` (which inserts a null entry).
- **C — Java/UI.** `renderer/DisplayXRenderer.java` (HostRenderer + the 3 listeners, thin shell — native owns presentation, `requestRender()` is a no-op). `XServerView.initRenderer("displayx")` branch on a plain SurfaceView. `"DisplayX"` added to `rendererEntries` (ContainerDetailViewModel.kt) + the ShortcutsScreen per-game mapping. API-29 gate in `XServerDisplayActivity.setupUI()` alongside ASR's. `PresentExtension.presentPixmap` gains a DisplayX branch FIRST in the chain — registers the AHB as a native direct content on first present (upstream registers in DRI3 `PixmapFromBuffers` where it knows the window; our DRI3 creates pixmaps standalone, so the window↔pixmap association only exists at present time), deduped via a `HashSet<Long>` of (windowId, drawableId).

**Status:** ⚠️ code complete for a first boot, **never compiled** (no local builds — CI only) and never run. Next: push → CI → fix compile errors → device-test.
**Known gaps by design:** no fences, no BGRA→RGBA conversion (Mali will likely show swapped channels), magnifier/filter-mode/unviewable-WM-class are no-ops, fullscreen is upstream's binary stretch toggle only, HUD tick not wired into the DisplayX present path.
