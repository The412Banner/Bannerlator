...
## 2026-08-04 — 🎞️✅ **bionic-fg 0.1.1 + FSR3 models 3/4 — COMPLETE, ⏸️ PAUSED EOD (resume AM)** (branch `feat/bionic-fg-0.1.1` @ `8a228a2d`, vc FROZEN 67)
> **⏸️ RESUME POINT (morning 2026-08-05):** user device-tests the staged APKs — install **`Bannerlator-fg011-m34-pubg.apk`** (models 0/1/3/4; 2 grayed) and check models 3 & 4 for ghosting / wrong-direction motion / edge shimmer. Then user reports observations → I TUNE the optical-flow constants (flow **sign** first, then `flowScale`, the ±0.5 prev/curr split, model-4 occlusion `kModel3OcclLo/Hi`, `of3_flow` search window) in the fork source, rebuild the r27d layer, re-bundle, re-stage. `flowScale` is also live-adjustable in the in-game FG menu without a rebuild. AFTER 3/4 device-proven: merge `feat/bionic-fg-0.1.1`→main (gate=device-proven), optional upstream PR of the NDK-<r27 const fix. Nothing is merged to main yet; vc stays 67.
>
> Full arc landed. **Exact-replica base:** bundled layer rebuilt to upstream v0.1.1 clean-room, **NDK r27d / clang 18** (matches Nick's toolchain), pure source (no const workaround). **Fork releases (The412Banner/bionic-fg):** `v0.1.1` = exact upstream replica (models 0/1, 5 stripped assets matching Nick byte-for-byte on glibc, 0.2% on arm64) — kept as fork "latest"; `v0.1.1-fsr3.1` (**prerelease**) = same base **+ our FSR3 models 3 & 4** (arm64 r27d stripped, glibc gcc-13.3 unstripped). **Models 3/4 re-layered CLEAN:** our own MIT-FidelityFX optical-flow GLSL slotted into v0.1.1's clean-room shader registry (companion generator, slots 56-61), riding the clean-room model-0 synthesis; **model 2 (traced Lossless + GameScope) intentionally omitted** so the "no third-party compiled artifacts" property holds. App UI: models 0/1/3/4 selectable, model 2 grayed. **Staged test APKs** (pkg `com.tencent.ig`, install over each other): A=our replica `Bannerlator-fg011-r27d-pubg.apk`; B=Nick's original `Bannerlator-fg011-orig-pubg.apk`; **models 3/4 `Bannerlator-fg011-m34-pubg.apk`** (in-APK `.so` `9cb88298` verified). ⏭️ **NEXT: user device-test** models 0/1/3/4 (3/4 UNVERIFIED — tune optical-flow sign/scale if ghosting). Then merge to main (gated on device-proven) + optional upstream PR of the NDK-<r27 portability fix. Cleanup: throwaway branch `abtest-upstream-orig-so`. → [[project_bannerlator_bionic_fg_011_upgrade]]
