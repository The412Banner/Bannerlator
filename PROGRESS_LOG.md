...
## 2026-08-20 — ✅🖼️ **Games wall MERGED TO MAIN — opt-in gate + account brand + scrollable outlined sheets**
> **Merge:** `main` `c3281b45` → **`ae765278`** (`--no-ff` from `feat/bigpicture-games-wall`; main was a clean direct ancestor, nothing to fold in). User: "merge it to main, we will polish it more over the weekend." Branch kept for the weekend pass. No versionCode bump (dev builds frozen). CI-green all 3 flavors; round-2 polish is device-UNCONFIRMED (merged on user's say-so before an on-device pass).
> **Gate fix (`3adfb908`):** the wall had been wired as a FORCED library replacement, so fresh installs booted straight into it (user didn't want that). Now **opt-in behind `enable_big_picture_mode`**: `Screen.Games` restored to the phone-grid `ShortcutsScreen()`; the wall renders on `Screen.BigPicture` (the route the toggle already targets in `MainActivity`); `isFullBleed` trimmed to `isBigPicture` only so the restored library keeps its chrome. Old `BigPictureScreen` **retired** (no longer routed — user: "wall replaces old BP") but the FILE stays because `GamesWallScreen` reuses its same-package `loadCover`/`launchShortcut`. Net: install → phone library; toggle ON → games wall.
> **Polish round 2 (`f6c4dc2a`), all in `GamesWallScreen.kt`:**
> • **Header brand = account entry point.** Top-left shows the signed-in user's `AccountAvatar` + username, or the app logo (bolt) + "Bannerlator" when logged out (fallback = app logo, per user — NOT AccountAvatar's Person-icon default). Tap → the existing wall `MyAccountDialog` (login/create/logout/profile). Live via `AccountUiBus` (mirrors `AccountManager` as Compose state; refreshed on entry + sheet-dismiss; the dialog refreshes it on every auth/avatar change).
> • **Tools/Power/Options sheets now scroll** — `WallSheetScaffold` was a plain `Column`, so lower Tools rows (Downloads/Settings) fell below the fold with NO touch scroll and D-pad focus walked off-screen. Added `verticalScroll` (fixes touch) + per-row `BringIntoViewRequester` keyed to `focusIndex` (fixes D-pad follow). `@OptIn(ExperimentalFoundationApi::class)`.
> • **App outlined-menu chrome on the sheets** — 1dp `colorScheme.outline` border + `surfaceContainer` bg + top-rounded shape (matches `Modifier.outlinedMenuCard()` / `OutlinedAlertDialog`), plus thin `HorizontalDivider(color = outline)` between options and under the title (user asked for both the outline and the inter-option grey line "like menus in Bannerlator").
> **Weekend follow-ups:** on-device confirm of round-2 (brand swap on login, Tools scroll via touch+D-pad, sheet outline/dividers); two stray working-tree edits `store/SteamGamesActivity.kt`+`store/SteamRepository.java` (unrelated, kept OUT of every wall commit — awaiting user's call). → memory `project_bannerlator_bigpicture_games_wall`.
