...
- ▶️ **NEXT:** pause agent CI-green → merge `feat/reshade-pause-pulse` into `feat/reshade-multi-effect-switch` → build **one APK** → **DEVICE TEST** (the real gate: per-effect toggle live on Vulkan/DXVK, Solo/Stack switching, pulse + pause-box, Live-preview toggle, migration of an existing single-effect profile). Then merge decision, then Tier 2 (live add-from-catalog with pause-assisted recompile). No merge / no release / no version bump without user go.
