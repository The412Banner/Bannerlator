...
## 2026-07-06 — 🐞→✅ CHECKPOINT: DepotSizeResolver "executor rejected" download-hang FIXED (`2b195ef`, CI green) + DB-downgrade crash resolved

> **Session on the DepotSizeResolver build (`feat/depot-size-resolver`). Two device bugs found & handled; logcat bridge confirmed up (verbs = `sh <cmd>`/`exec <cmd>` only; device = AYANEO Pocket FIT, `com.winlator.banner` = standard flavor).**
>
> **BUG 1 — Steam menu crash (DB downgrade), RESOLVED by roll-forward.** Opening the Steam side-menu item hard-crashed: `SQLiteException: Can't downgrade database from version 4 to 3` at `SteamDatabase.queryGames:296` (`SteamGamesActivity.onCreate:206`) + a 2nd FATAL on `SteamLibraryWorker` via `clearLicenses:358`. Cause: DepotSizeResolver's additive v3→4 migration had bumped on-disk `steam.db` to v4, then user reinstalled **stable 2.3 (vc38, knows only v3)** over it → default `onDowngrade` throws. **Fix applied = roll forward:** reinstalled the DepotSizeResolver v4 APK (07:16), Steam screen works, device now ON the v4 build. Hardening TODO (not done): give `SteamDatabase` a real `onDowngrade` so any future v4→earlier rollback can't brick the Steam screen.
>
> **BUG 2 — Brawlhalla stuck at 0% (`executor rejected`), ROOT-CAUSED + FIXED `2b195ef`.** Download hung at 0% in an infinite tight retry loop; every CDN manifest fetch failed instantly with `java.io.InterruptedIOException: executor rejected` (device-repro: appId **291550** / depot **228988**; cycled cache2/9/11/… endlessly, saturated logcat → force-stopped to halt CPU/battery burn). **Root cause:** `DepotSizeResolver.resolveOnWorker()` `finally` called **`cdn.close()`**. `CdnClient(steamClient)` wraps the SteamClient's **app-wide SHARED OkHttpClient**; `close()` shuts down that dispatcher's ExecutorService — which the REAL depot downloader (`DepotDownloader.downloadManifest`) reuses → poisoned for the process. Sequence: open detail page → `maybeResolveRealSize`→resolver runs (gated OFF during active downloads) → `cdn.close()` kills executor → Install → downloader hits dead executor → 0% hang. **Fix = remove `cdn.close()`** (downloader engine never closes the shared client; resolver's cdn owns nothing to free; kept watchdog stop + `scope.cancel()`). Commit **`2b195ef`** (was `1f262e3`), pushed; CI **manual-dispatch** run **`28788173770` GREEN (success)**. NOT on main → stable 2.3 unaffected.
>
> **NEXT (device-test the green build):** reinstall `2b195ef` APK → verify (1) Brawlhalla downloads past 0%, (2) open-detail-then-download works generally, (3) THEN the still-unproven original install-blocker repro (appId **313830** / depot **313831**, devaspe's over-report game). No versionCode change (stays vc38); whenever a stable is cut → 2.4-preN, bump vc→39+.
