...
## 2026-08-07 — 🎮 CONTROLLER OVERHAUL (P0–P5) + Free Memory dual-tier — branch `feat/controller-hotplug` (NOT merged; awaiting device test)

Full input-stack rework driven by two user-reported bugs (on-screen touch input dying ~15 min in; an AYANEO handheld's stick-less aux device stealing XInput Player 1) plus the long-standing #221 controller drops. Sources: **WinNative** (mmap-ring transport + descriptor slots), **GameNative** (`onInputDeviceChanged` progressive-capability), **palazos** (confirmed a subset of the hot-plug hooks). Final build run `31194608138`, HEAD `389398f7`. pubg APK staged `/sdcard/Download/Bannerlator-controller-hotplug-pubg.apk`.

- **P0 `f02b76f3`** — Replaced the append-file fake-evdev transport with a fixed **mmap ring buffer + seqlock keyframe** (`FakeInputWriter.java` + native `fakeinput.cpp`, shared FIBR v2 header). Fixes the ~15-min on-screen touch-input death (no short-write desync, no swallowed IOException, no unbounded file/ENOSPC). Rumble EV_FF→`winlator_vibration` path byte-identical. P0-only CI build was green before layering the rest.
- **P1 `d4b7bfd9`** — Descriptor-keyed slots (`descriptorToSlot`/`deviceToDescriptor`), filled `onInputDeviceAdded/Changed/Removed`, 400ms disconnect debounce, `preAssignConnectedControllers` before Wine boots, sibling sub-device grouping, broadened fingerprint guard (`uinput-fpc`/`goodix_fp`/`uinput-`). Reconnected pad returns to its same slot.
- **P3 `62b5fe61`** (+`fa1e8621` @OptIn fix) — Capability gamepad filter (must have real analog axes OR BTN_GAMEPAD buttons → rejects the AYANEO aux, safe for arcade/fight sticks & normal phones). In-game **Controls → Players** tab (Auto/Player 1-4/Ignore, live). **Reset Input** button (`resetInputPipeline`).
- **P4 `1a9473d4`** (+`f42dcbdd` import fix) — Out-of-game **container + shortcut Player-Slots settings** (shared JSON schema `WinHandler.parse/buildSlotOverridesJson`); shortcut overrides container; deterministic slots at launch.
- **P5a `67d209ab`** — Shared/combined slot (touch+pad → one player, **manual-only**, per-slot state merge so OSC neutral can't stomp a held pad) + OSC auto-yield mode (KEEP/YIELD/SHARE, per-container) + **app-drawer global default** Player-Slots (seeds NEW containers only) + "?" help on all 3 surfaces.
- **P5b `b6b64fc3`** — In-game **controller-status toast overlay** (`ControllerToastOverlay.kt`), pixel-matched to the user-approved mockup across 6 states (launch/hotplug/shared/full-house/ignored), 280/3200/450ms, non-interactive, fires via a UI-agnostic `WinHandler.ControllerAssignmentListener` (main-looper, 300ms debounce). + thin dividers between in-game Players rows.
- **P5c `389398f7`** — **Free Memory dual-tier** (fixes POCO F5/HyperOS "does nothing" report): Tier 1 relabeled "Drop file caches" (honest); Tier 2 root **`am kill-all`** deep-clean (spares foreground game FGS + system by OS rules — no force-stop/safelist), as a manual button (settings + in-game) AND an "Auto deep-clean on launch" toggle. New `RootManager.runCommand`.

Rumble (mode/intensity/per-slot tuning) and Processor Affinity both verified working device-side during this session. **NEXT: user device-tests the full stack → merge to main (device-proven gate).** Deferred/future: none blocking — earlier P2 (WinNative virtual-gamepad rebalance) intentionally skipped as risky/low-value.
