...
## 2026-08-15 — 🎮 **#345 controller FIX-4: independent per-controller profiles — in-game core built, apply-fix awaiting retest** (branch `fix/controller-slot-takeover-345` tip `bdc6ca4a`, CI `31877327849` green)
> **User chose "independent profile per controller":** each connected device (and OSC) carries its own controls profile so its bindings apply independently. Engine approach (no input-path rewrite): re-seed each assigned device's bindings from its chosen profile onto the ACTIVE profile's runtime controller instance (the one `onGenericMotionEvent→profile.getController(deviceId)` remaps on).
> **Landed:** Container `controllerProfiles` extra ({descriptor:profileId}, resolved container→shortcut); XServerDialogState `PlayerSlotRow.assignedProfileId` + `controllerProfileOptions` + `onControllerProfileChanged` (fun interface); XServerDrawer Players-tab per-device **Profile** picker beside the slot selector; XSDA load/persist/apply + hooks at launch, hot-plug tick, and assignment change.
> **Device test #1 (`ce2bb8db`, Fold 8):** ✅ Players-tab Profile picker works, ✅ assignment persists across relaunch — ❌ but the assigned bindings didn't FIRE (pad ran raw XInput). Root: `active.getController(descriptor)` doesn't auto-create; a fresh Virtual Gamepad profile has no runtime controller for the pad (empty Default template → gated seed) → no destination AND the input path got null → raw XInput. **Fix `bdc6ca4a`:** `ControlsProfile.getOrCreateController(deviceId)` (ungated) + `WinHandler.getDeviceIdForDescriptor()` + applyControllerProfiles hosts bindings on that instance. **⏳ AWAITING Fold 8 retest** (does d-pad→arrows fire now).
> **Build iteration:** 57459df2 (FAILED: Java void lambda vs Kotlin `->Unit`) → ce2bb8db (fun-interface, green, device apply-fail) → bdc6ca4a (apply-fix, green). **NEXT after retest:** App/container/shortcut DEFAULT profile selectors (out-of-game half), then FIX-5/FIX-3/Test-8/F2-Keep-Share. Full detail + resume steps in memory [[project_bannerlator_controller_slot_takeover_345]] "RESUME HERE".


## 2026-08-14 (late) — 🎮 **#345 controller — Fold 8 session: selector-coherence fix DEVICE-PROVEN, core slot system verified, next-build queue set** (branch `fix/controller-slot-takeover-345` tip `b3b7d446`, CI `31860812869` green)
> **Root fix landed + proven:** the in-game Controls profile selector read "-- Disabled --" while a live Virtual Gamepad overlay drove user 0. Cause: the drawer computes its selected-profile index ONCE in `initInlineTabStates`, which runs BEFORE `simulateConfirmInputControlsDialog` seeds the smart default → index stuck at 0, never updated by `showInputControls`. **Fix (`b3b7d446`):** `showInputControls`/`hideInputControls` now push the active profile's index to `XServerDialogState`. **Device-proven Fold 8:** selector now reads **Virtual Gamepad** consistently across relaunches.
> **✅ Device-proven this session (Fold 8 + real BT pad):** FIX-1/2 selector coherence · **F5 disconnect→restore** (OSC returns, drives user 0, labeled P1 — never verified before) · F5b Ignore→restore · F7/F7c mid-game hand-over+hide. Core #345 slot system (F1/F2/F5/F6/F7/F7c) confirmed.
> **🎯 NEXT BUILD (queued, user out of tokens til ~08-15 eve):** FIX-4 (per-controller controls-profile selection: App Input Controls per-connected-controller selector w/ bundled "Virtual Gamepad" default → container inherits → shortcut inherits → honored at launch + in-game Players-tab per-controller profile picker beside the slot selector, remembered per shortcut) · FIX-5 (OSC must RELEASE its slot on hand-over, not land on P2 — repro: OSC displayed at connect) · FIX-3 (relaunch Players-tab label 'unassigned'→Player 1) · Test 8 pad-at-launch (OSC doesn't hide) · first-launch side-menu-hide quirk · F2 Keep/Share still untested (needs mid-game connect). Full detail + file map in memory [[project_bannerlator_controller_slot_takeover_345]] "NEXT SESSION — START HERE".


## 2026-08-14 — 🎮 **#345 controller — Fold 8 guided test session + FIX-1/2/4-core** (branch `fix/controller-slot-takeover-345`, rebased onto main `d883d01d` → tip `ea238949`, CI `31859186758`)
> **Rebase:** the 5-commit #345 branch was 51 behind main; rebased clean (only PROGRESS_LOG doc conflicts, kept main's), CI-green, tip `5aff704f`. Then FIX-1/2/4-core on top = `ea238949`.
> **Guided test session (Samsung Fold 8 + BT pad, fresh install, defaults; user drives, no logcat/bridge on Fold 8):**
> - ✅ **TEST 3 PASS** — BT pad connects mid-game → toast "assigned to Player 1", OSC HID, pad drives user 0. **F7/F7c device-proven on Fold 8.** Defaults correct (app=Hand over & hide, new container=Use global default).
> - ✅ **Keyboard bindings WORK** — user made profile "test one" on the BT pad (d-pad→arrows etc.); with it selected in-game the pad moved drive selection on the Wine desktop. **The reporter's "bindings don't work" bug = there's no way to make a profile ACTIVE by default; the pad ran raw XInput.** Per-device bindings already persist at the device level once set.
> - 🔴 **Root incoherence (blocked F5/F6/F2 testing):** smart-default seeds the VG overlay (drives user 0) but the in-game selector reads **"Disabled"**, doesn't persist across relaunch, and seeding became inconsistent. Players tab labels the OSC slot "Auto/unassigned" not Player 1.
> - 🐛 **Touch bug (FIX-5):** touching the OSC before connecting a pad promotes it into a slot → pad lands P1, OSC stays P2 (no hide). Untouched → correct hand-over.
> **FIX-1/2/4-core LANDED (`ea238949`):** smart-default now **persists the Virtual Gamepad by its stable id** when seeded → the in-game selector reflects "Virtual Gamepad" (was Disabled), it's **honored at next launch** (a default profile that sticks = FIX-4 launch-honor for VG), timeout stays OFF by default. This is the profile-state-coherence fix that unblocks clean F5/F6/F2 testing.
> **▶️ NEXT ITERATION (phased, not in this build):** FIX-3 (in-game Players-tab shows OSC's real slot = Player 1, not unassigned), FIX-5 (touch must not promote OSC into a slot), FIX-4 full UI (default-profile selectors at App Input Controls / container / shortcut, + per-controller/per-OSC profile picker in the Players tab, + in-game assignment remembered per game shortcut). Full detail in memory [[project_bannerlator_controller_slot_takeover_345]].
