...
**Round 3 (separate commit, same branch):** PE icon extractor as fallback when SteamGridDB has nothing. New `ExeIconExtractor.kt` (~280 lines, no third-party deps) parses the DOS+PE+optional headers, locates the resource section, walks the resource tree to `RT_GROUP_ICON`, builds a candidate list by cross-referencing `RT_ICON` entries by ID, picks the entry closest to 64×64 (tiebreaker: higher bit depth), and decodes either PNG-embedded (Vista+ ≥256px) or 32/24-bit BITMAPINFOHEADER DIB pixel data. Wired into `importExe`'s background cover-art thread: runs only after `StarLaunchBridge.saveCoverArt` returns and the `<safeName>.png` still does not exist in `container.getIconsDir(64)`. On hit, saves to both that dir and `Shortcut.saveCustomCoverArt(...)`. Paletted icons (8/4/1-bit) return null and the shortcut just gets no icon — rare on modern EXEs.
