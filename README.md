...
The two **native** engines are the ones to reach for on a game you've capped: lock the game at **30**, pick **4×**, and the panel gets **120** — with the game itself untouched, the device cooler than running it uncapped, and the in-game HUD showing `30→120`. They work best at or under **panel ÷ 4** (36 fps on a 144 Hz screen fills every refresh). **Win-FG Native** needs no files at all; **LSFG Native** runs the Lossless Scaling shaders from your own DLL. **lsfg-vk** multiplies the game's *own* frame counter instead, which is what you want for uncapped benchmarking.
...
| **Mailbox** | "Fast vsync" — never makes the game wait, still tear-free. The right mode for lsfg-vk, so its extra frames aren't throttled on the way out. |
| **Immediate** | "Vsync off" — lowest input lag, but can tear. |

The engines want **opposite** present modes, and Bannerlator handles it for you. **The native engines force FIFO** while generating — it queues the real and generated frames together and needs FIFO to show them one per refresh; under Mailbox the display would keep only the newest and discard the rest. lsfg-vk prefers Mailbox for the mirror-image reason. Your chosen mode is restored the moment frame generation turns off. While a native engine is generating it also **locks the FPS limiter on** and **Auto refresh (VRR) off** — that's the configuration it was proven in, and both controls come back as you had them when it stops. You can also switch modes live from the **Present Mode selector** in the in-game Graphics tab, and every mode is explained by a **"?"** button and in the in-app **"What is all this?"** glossary.
...
- Bannerlator's HUD counts the frames the **game** delivers. With a **native engine** (Win-FG Native or LSFG Native) the generated frames are added *after* that point, so every HUD style shows both numbers — **`30→120`** — the game's rate and what actually reaches the panel. With lsfg-vk the same arrow appears only if frames are being *lost* between the game and the panel (e.g. `70→35`), which is a problem rather than a feature.
