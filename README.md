...
| **Version** | Bannerlator **V 3.0.6** — built from Star **marcescence** (`versionName 3.0.6`, `versionCode 80`) |
...
- **Two selectable frame-generation engines** — pick **Off / win-fg / LSFG Native** per container; the running engine is shown as a badge in the in-game drawer.
  ...
  - **LSFG Native** *(new in 3.0.6)* — Lossless Scaling's frame generation running **inside Bannerlator's own compositor**, ported from the **Eden** emulator's native present path with **WinNative**'s implementation as the reference. The shaders come from *your own* `Lossless.dll`, extracted on device. Device-proven 30 → 120 fps at the panel. *(The former **lsfg-vk** in-game layer engine is retired; anything still set to it runs LSFG Native.)*
