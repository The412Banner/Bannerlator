...
pub struct BlCmPersonaEvent {
    ...
    pub rp_pairs: *const BlCmRichPresenceKV,
    pub rp_count: usize,
}

#[repr(C)]
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct BlCmLicenseEntry {
    pub package_id: u32,
    pub owner_id: u32,
...
pub struct BlCmLobbyData {
    ...
    pub metadata_bytes: *const u8,
    pub metadata_len: usize,
    pub members: *const BlCmLobbyMember,
    pub member_count: usize,
}

pub type BlCmPersonaObserverFn = Option<extern "C" fn(*const BlCmPersonaEvent)>;
pub type BlCmLogonStateObserverFn = Option<extern "C" fn(bool)>;
pub type BlCmFriendsListObserverFn = Option<extern "C" fn(*const u64, usize)>;
pub type BlCmLicenseListObserverFn = Option<extern "C" fn(*const BlCmLicenseEntry, usize)>;
pub type BlCmAccountInfoObserverFn = Option<extern "C" fn(*const BlCmAccountInfo)>;
pub type BlCmServerRealTimeObserverFn = Option<extern "C" fn(u32)>;
pub type BlCmLobbyListCb = Option<extern "C" fn(u64, i32, *const BlCmLobbyEntry, usize)>;
pub type BlCmLobbyDataObserverFn = Option<extern "C" fn(*const BlCmLobbyData)>;
pub type BlCmLobbyCreatedCb = Option<extern "C" fn(u64, i32, u64)>;
pub type BlCmLobbyJoinedCb = Option<extern "C" fn(u64, i32, u64)>;
pub type BlCmLobbySetDataCb = Option<extern "C" fn(u64, i32)>;
pub type BlCmLobbySetOwnerCb = Option<extern "C" fn(u64, i32)>;
pub type BlCmLobbyChatMsgObserverFn = Option<extern "C" fn(u64, u64, *const u8, usize)>;
pub type BlCmLobbyMembershipObserverFn = Option<extern "C" fn(i32, u64, u64, *const c_char)>;

#[derive(Default)]
pub struct CmBridgeObservers {
    persona: Mutex<BlCmPersonaObserverFn>,
    logon_state: Mutex<BlCmLogonStateObserverFn>,
    friends_list: Mutex<BlCmFriendsListObserverFn>,
    license_list: Mutex<BlCmLicenseListObserverFn>,
    account_info: Mutex<BlCmAccountInfoObserverFn>,
    server_realtime: Mutex<BlCmServerRealTimeObserverFn>,
    lobby_data: Mutex<BlCmLobbyDataObserverFn>,
    lobby_chat_msg: Mutex<BlCmLobbyChatMsgObserverFn>,
    lobby_membership: Mutex<BlCmLobbyMembershipObserverFn>,
}

impl CmBridgeObservers {
    pub fn register_persona(&self, callback: BlCmPersonaObserverFn) {
        *self.persona.lock().expect("persona observer poisoned") = callback;
    }

    pub fn dispatch_persona(&self, event: &BlCmPersonaEvent) {
        if let Some(callback) = *self.persona.lock().expect("persona observer poisoned") {
            callback(event);
        }
    }

    pub fn register_logon_state(&self, callback: BlCmLogonStateObserverFn) {
    ...
    pub fn register_friends_list(&self, callback: BlCmFriendsListObserverFn) {
    ...
    pub fn register_license_list(&self, callback: BlCmLicenseListObserverFn) {
        *self.license_list.lock().expect("license observer poisoned") = callback;
    }

    pub fn dispatch_license_list(&self, licenses: &[BlCmLicenseEntry]) {
        if let Some(callback) = *self.license_list.lock().expect("license observer poisoned") {
            callback(licenses.as_ptr(), licenses.len());
        }
    }

    pub fn register_account_info(&self, callback: BlCmAccountInfoObserverFn) {
        *self.account_info.lock().expect("account observer poisoned") = callback;
    }

    pub fn dispatch_account_info(&self, info: &BlCmAccountInfo) {
        if let Some(callback) = *self.account_info.lock().expect("account observer poisoned") {
            callback(info);
        }
    }

    pub fn register_server_realtime(&self, callback: BlCmServerRealTimeObserverFn) {
        *self
            .server_realtime
            .lock()
    ...
    pub fn register_lobby_data(&self, callback: BlCmLobbyDataObserverFn) {
        *self
            .lobby_data
            .lock()
    ...
    pub fn dispatch_lobby_data(&self, data: &BlCmLobbyData) {
        if let Some(callback) = *self
            .lobby_data
            .lock()
    ...
    pub fn register_lobby_chat_msg(&self, callback: BlCmLobbyChatMsgObserverFn) {
        *self
            .lobby_chat_msg
            .lock()
    ...
    pub fn register_lobby_membership(&self, callback: BlCmLobbyMembershipObserverFn) {
        *self
            .lobby_membership
            .lock()
...
impl CmBridge {
    ...
    pub fn dispatch_persona_friend(&self, friend: &PersonaStateFriend) {
        ...
        let pairs = key_values
            .iter()
            .map(|(key, value)| BlCmRichPresenceKV {
                key: key.as_ptr(),
                value: value.as_ptr(),
            })
            .collect::<Vec<_>>();
        let event = BlCmPersonaEvent {
            sid: friend.friendid,
    ...
    pub fn dispatch_lobby_data_message(&self, msg: &CMsgClientMMSLobbyData) {
        ...
        let members = msg
            ...
            .map(|(member, name)| BlCmLobbyMember {
...
impl From<&License> for BlCmLicenseEntry {
...
#[no_mangle]
pub extern "C" fn bl_cm_set_persona_state(persona_state: i32) -> bool {
    if persona_state < 0 {
        return false;
    }
...
#[no_mangle]
pub unsafe extern "C" fn bl_cm_set_persona_name(name: *const c_char, persona_state: i32) -> bool {
...
#[no_mangle]
pub extern "C" fn bl_cm_request_user_info(steam_id: u64, flags: i32) -> bool {
    if steam_id == 0 {
        return false;
    }
    let Some(client) = global_bridge().active() else {
...
#[no_mangle]
pub unsafe extern "C" fn bl_cm_request_user_info_bulk(
    sids: *const u64,
    count: usize,
    _flags: i32,
) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn bl_cm_get_cached_app_ownership_ticket(
    app_id: u32,
    out_buf: *mut u8,
    max_len: usize,
    out_len: *mut usize,
...
#[no_mangle]
pub unsafe extern "C" fn bl_cm_bridge_inject_test_ownership_ticket(
    app_id: u32,
    bytes: *const u8,
    len: usize,
) -> bool {
...
#[no_mangle]
pub extern "C" fn bl_cm_notify_games_played(app_id: u32) -> bool {
    let Some(client) = global_bridge().active() else {
        return false;
    };
...
#[no_mangle]
pub unsafe extern "C" fn bl_cm_set_rich_presence(
    app_id: u32,
...
#[no_mangle]
pub unsafe extern "C" fn bl_cm_store_user_stats(
    app_id: u32,
    _crc_stats: u32,
    stat_ids: *const u32,
...
#[no_mangle]
pub extern "C" fn bl_cm_bridge_register_persona_observer(callback: BlCmPersonaObserverFn) {
    global_bridge().observers.register_persona(callback);
}

#[no_mangle]
pub unsafe extern "C" fn bl_cm_bridge_dispatch_persona(event: *const BlCmPersonaEvent) {
...
#[no_mangle]
pub extern "C" fn bl_cm_bridge_register_logon_state_observer(callback: BlCmLogonStateObserverFn) {
    global_bridge().observers.register_logon_state(callback);
}

#[no_mangle]
pub extern "C" fn bl_cm_bridge_dispatch_logon_state(logged_on: bool) {
    global_bridge().observers.dispatch_logon_state(logged_on);
}

#[no_mangle]
pub extern "C" fn bl_cm_bridge_inject_test_logon_state(logged_on: bool) {
    bl_cm_bridge_dispatch_logon_state(logged_on);
}

#[no_mangle]
pub extern "C" fn bl_cm_bridge_register_friends_list_observer(callback: BlCmFriendsListObserverFn) {
    global_bridge().observers.register_friends_list(callback);
}

#[no_mangle]
pub unsafe extern "C" fn bl_cm_bridge_dispatch_friends_list(sids: *const u64, count: usize) {
    if count == 0 {
...
#[no_mangle]
pub unsafe extern "C" fn bl_cm_bridge_inject_test_friends_list(sids: *const u64, count: usize) {
    unsafe { bl_cm_bridge_dispatch_friends_list(sids, count) };
}

#[no_mangle]
pub extern "C" fn bl_cm_bridge_register_license_list_observer(callback: BlCmLicenseListObserverFn) {
    global_bridge().observers.register_license_list(callback);
}

#[no_mangle]
pub unsafe extern "C" fn bl_cm_bridge_dispatch_license_list(
    licenses: *const BlCmLicenseEntry,
    count: usize,
) {
    if count == 0 {
...
#[no_mangle]
pub unsafe extern "C" fn bl_cm_bridge_inject_test_license_list(
    licenses: *const BlCmLicenseEntry,
    count: usize,
) {
    unsafe { bl_cm_bridge_dispatch_license_list(licenses, count) };
}

#[no_mangle]
pub extern "C" fn bl_cm_bridge_register_account_info_observer(callback: BlCmAccountInfoObserverFn) {
    global_bridge().observers.register_account_info(callback);
}

#[no_mangle]
pub unsafe extern "C" fn bl_cm_bridge_dispatch_account_info(info: *const BlCmAccountInfo) {
...
#[no_mangle]
pub unsafe extern "C" fn bl_cm_bridge_inject_test_account_info(info: *const BlCmAccountInfo) {
    unsafe { bl_cm_bridge_dispatch_account_info(info) };
}

#[no_mangle]
pub extern "C" fn bl_cm_bridge_register_server_realtime_observer(
    callback: BlCmServerRealTimeObserverFn,
) {
    global_bridge().observers.register_server_realtime(callback);
}

#[no_mangle]
pub extern "C" fn bl_cm_bridge_dispatch_server_realtime(server_realtime: u32) {
    global_bridge()
        .observers
...
#[no_mangle]
pub extern "C" fn bl_cm_bridge_register_lobby_data_observer(callback: BlCmLobbyDataObserverFn) {
    global_bridge().observers.register_lobby_data(callback);
}

#[no_mangle]
pub extern "C" fn bl_cm_bridge_register_lobby_chat_msg_observer(
    callback: BlCmLobbyChatMsgObserverFn,
) {
    global_bridge().observers.register_lobby_chat_msg(callback);
}

#[no_mangle]
pub extern "C" fn bl_cm_bridge_register_lobby_membership_observer(
    callback: BlCmLobbyMembershipObserverFn,
) {
    global_bridge()
        .observers
...
#[no_mangle]
pub extern "C" fn bl_cm_bridge_start_state_sync_poller() {
    state_sync_poller_running().store(true, Ordering::Release);
}

#[no_mangle]
pub extern "C" fn bl_cm_bridge_stop_state_sync_poller() {
    state_sync_poller_running().store(false, Ordering::Release);
}

#[no_mangle]
pub unsafe extern "C" fn bl_cm_lobby_get_list(
    hcall: u64,
    app_id: u32,
...
#[no_mangle]
pub extern "C" fn bl_cm_lobby_create(
    hcall: u64,
    app_id: u32,
    _lobby_type: i32,
    _max_members: i32,
    callback: BlCmLobbyCreatedCb,
) -> bool {
    let Some(client) = global_bridge().active() else {
        return false;
    };
...
#[no_mangle]
pub extern "C" fn bl_cm_lobby_join(
    hcall: u64,
    _app_id: u32,
    lobby_sid: u64,
    callback: BlCmLobbyJoinedCb,
) -> bool {
    let Some(client) = global_bridge().active() else {
        return false;
    };
...
#[no_mangle]
pub extern "C" fn bl_cm_lobby_leave(app_id: u32, lobby_sid: u64) -> bool {
    let Some(client) = global_bridge().active() else {
        return false;
    };
...
#[no_mangle]
pub unsafe extern "C" fn bl_cm_lobby_send_chat(
    _app_id: u32,
    lobby_sid: u64,
    data: *const u8,
    len: usize,
...
#[no_mangle]
pub unsafe extern "C" fn bl_cm_lobby_set_data(
    hcall: u64,
    _app_id: u32,
    lobby_sid: u64,
...
#[no_mangle]
pub extern "C" fn bl_cm_lobby_set_owner(
    hcall: u64,
    _app_id: u32,
    lobby_sid: u64,
    new_owner_sid: u64,
    callback: BlCmLobbySetOwnerCb,
) -> bool {
    let Some(client) = global_bridge().active() else {
        return false;
    };
...
#[no_mangle]
pub extern "C" fn bl_cm_lobby_invite_user(app_id: u32, lobby_sid: u64, invitee_sid: u64) -> bool {
    let Some(client) = global_bridge().active() else {
        return false;
    };
...
pub fn lobby_state_path(dir: &Path, app_id: u32) -> PathBuf {
    dir.join(format!("bl_lobby_{app_id}.txt"))
}

pub fn lobby_request_path(dir: &Path, app_id: u32) -> PathBuf {
    dir.join(format!("bl_lobby_req_{app_id}.txt"))
}
...
pub fn parse_lobby_state_file(path: &Path) -> std::io::Result<(i32, Vec<BlCmLobbyEntry>)> {
...
fn try_lobby_list_from_file(
    hcall: u64,
    app_id: u32,
    callback: extern "C" fn(u64, i32, *const BlCmLobbyEntry, usize),
) -> bool {
    let dir = state_dir();
...
mod tests {
    ...
    extern "C" fn license_cb(licenses: *const BlCmLicenseEntry, count: usize) {
        if count != 0 {
    ...
    extern "C" fn persona_cb(event: *const BlCmPersonaEvent) {
        let event = unsafe { event.as_ref() }.unwrap();
        PERSONA_ID.store(event.sid, Ordering::SeqCst);
    }

    extern "C" fn account_cb(info: *const BlCmAccountInfo) {
    ...
    extern "C" fn lobby_data_cb(data: *const BlCmLobbyData) {
    ...
    fn observer_dispatches_match_c_bridge_contract() {
        ...
        bridge
            .observers()
            .dispatch_license_list(&[BlCmLicenseEntry {
                package_id: 480,
                ..Default::default()
            }]);
    ...
    fn state_sync_poller_exports_track_running_state() {
        bl_cm_bridge_start_state_sync_poller();
        assert!(state_sync_poller_running().load(Ordering::Acquire));
        bl_cm_bridge_stop_state_sync_poller();
        assert!(!state_sync_poller_running().load(Ordering::Acquire));
    }
    ...
    fn bridge_commands_enqueue_outbound_cm_messages() {
        ...
        assert!(bl_cm_set_persona_state(1));
        assert!(bl_cm_notify_games_played(480));

        let name = CString::new("Ada").unwrap();
        assert!(unsafe { bl_cm_set_persona_name(name.as_ptr(), 1) });
        assert!(unsafe {
            bl_cm_lobby_get_list(
                10,
                480,
                50,
                ptr::null(),
        ...
        assert!(bl_cm_lobby_create(11, 480, 2, 4, None));
        assert!(bl_cm_lobby_leave(480, 123));
    ...
    fn lobby_get_list_callback_reads_existing_state_file() {
        ...
        let old = env::var_os("BL_STATE_DIR");
        env::set_var("BL_STATE_DIR", &dir);
        ...
        if let Some(old) = old {
            env::set_var("BL_STATE_DIR", old);
        } else {
            env::remove_var("BL_STATE_DIR");
        }
        let _ = fs::remove_dir_all(dir);
    }
}
