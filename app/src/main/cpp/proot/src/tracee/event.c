...
static void kill_all_tracees2(int signum, siginfo_t *siginfo UNUSED,
  ...
  note(NULL, WARNING, INTERNAL, "signal %d received from process %d", signum,
       siginfo->si_pid);
  kill_all_tracees();
...
static void print_talloc_chunk(const void *ptr, int depth, int max_depth UNUSED,
  ...
  if (is_ref)
    ...
    if (name[0] == '@') {
      ...
      fprintf(stderr, "\t(pid = %d, parent = %p)", ((Tracee *)ptr)->pid,
              ((Tracee *)ptr)->parent);
...
int handle_tracee_event(Tracee *tracee, int tracee_status) {
  ...
  if (WIFEXITED(tracee_status)) {
    ...
    VERBOSE(tracee, 1, "pid %d: exited with status %d", tracee->pid,
            last_exit_status);
    terminate_tracee(tracee);
  } else if (WIFSIGNALED(tracee_status)) {
    VERBOSE(tracee, (int)(tracee->pid != 1),
            "pid %d: terminated with signal %d", tracee->pid,
    ...
    case SIGTRAP | 0x80:
      ...
      case DISABLED:
        if (!tracee->seccomp_already_handled_enter) {
          ...
          if (tracee->chain.suppressed_signal &&
              tracee->chain.syscalls == NULL) {
    ...
    case SIGTRAP | PTRACE_EVENT_SECCOMP << 8: {
      ...
      if (seccomp_after_ptrace_enter && !IS_IN_SYSENTER(tracee)) {
        ...
        /* "!IS_IN_SYSENTER(tracee)" in condition above means we have pending
         * sysexit, so requesting to not be notified about it makes no sense */
    ...
    default:
      ...
      if (tracee->chain.syscalls != NULL) {
        VERBOSE(tracee, 5,
                "pid %d: suppressing signal during chain signal=%d, prev "
                "suppressed_signal=%d",
