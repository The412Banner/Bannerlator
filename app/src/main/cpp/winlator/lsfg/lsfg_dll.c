// Lossless.dll PE/RCDATA shader extractor + on-device SPIR-V cache.
// Ported verbatim from WinNative PR #697 "Feature/lsfg frame gen" (GPL-3.0-or-later).
// Never bundles, downloads, or redistributes any part of Lossless.dll — the user's
// own paid copy is parsed on device, and only the translated SPIR-V is cached.
#include "lsfg_dll.h"
#include "lsfg_dxbc.h"
...
#define LSFG_LOGW(...) __android_log_print(ANDROID_LOG_WARN, LOG_TAG, __VA_ARGS__)
...
LsfgStatus lsfg_load_modules(const char* cache_path, LsfgModuleSet* out_set) {
    ...
    out_set->variant = (LsfgVariant)header.variant;
