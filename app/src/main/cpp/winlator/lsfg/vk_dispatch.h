...
// Bannerlator: populate the dispatch table from an already-resolved
// vkGetInstanceProcAddr. The compositor loads the Vulkan driver itself (possibly
// a custom adrenotools driver living in an isolated linker namespace), so it owns
// the GetInstanceProcAddr chosen at dlopen time; feed it straight in rather than
// dlopen'ing libvulkan a second time. Follow with vkd_load_instance(instance).
bool vkd_init_from_gipa(PFN_vkGetInstanceProcAddr gipa);
