...
class BlConnection : AutoCloseable {
    ...
    fun setObserver(observer: BlConnectionObserver?) {
        val h = nativeHandle.get()
        if (h == 0L) return
    ...
    companion object {
        ...
        @JvmStatic private external fun nativeSetObserver(handle: Long, observer: BlConnectionObserver?)
