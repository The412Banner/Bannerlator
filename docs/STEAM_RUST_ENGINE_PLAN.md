...
| **2-A — depot install / resume / verify on the engine (flag ON)** | **IMPLEMENTED, not device-proven.** `store/BlDepotInstaller` (selected by `SteamDepotDownloader.installApp/resumeApp` when the flag is ON; JavaSteam path byte-identical when OFF) drives `BlSteamSession.downloadApp` — CM depot keys + manifest request codes → CDN manifest (server rotation, backoff) → chunk-hash-validated `depot_writer`. Same surface: `DownloadControl` (pause/cancel), `DownloadProgress:`/`DownloadComplete:`/`DownloadFailed:`/`DownloadPaused:`/`DownloadCancelled:` events, `DownloadRegistry` row, `steam_downloads` row, `markInstalled` + `.bannerlator_build`; install dir rules identical (resume dir → installed dir → "Install to SD card" root → internal); depots = `depot_manifests` rows (Phase 1-A grouping) minus opted-out DLC + stale-dupes; a fresh single-app PICS read precedes every pass so manifests are live. **Journal:** `<installDir>/.bl_depot/` (crate `depot_downloader::CONFIG_DIR_NAME`, deliberately NOT `.DepotDownloader/`): `depot.config` installed-manifest-per-depot (+ in-progress sentinel), cached manifests, clean-pause markers, `denied.depots`. **Completion = journal check:** installed iff every selected depot is recorded at exactly the requested manifest id (a depot is recorded only after every chunk hash-validated and every file truncated to manifest size) — bytes-this-pass never decide. **Verify** = `fresh=true` (journal forgotten for those depots → every chunk of every depot re-hashed, mismatches re-fetched). Resume across restart/network drop = journal + per-chunk hash skip. One native download at a time (queue). Denied DLC depots are auto-excluded; denied base depots fail honestly. Session-loss/CDN failures retry as resume (2×, backoff). Log tag `BL_STEAM_DL` (+ `steam_debug.txt`). Deferred: password-protected beta branches (honest refusal), compressed-bytes bar (follows the install fraction). | branch p2 |
| **2-B — game updates on the engine (flag ON)** | **IMPLEMENTED, not device-proven.** `SteamGameUpdater.checkForUpdate` (the launch pre-flight's "Game files" row + the launch popup's "Check for updates") now runs a FRESH single-app PICS refresh on the Rust engine (bounded 8 s; falls back to the synced build on timeout) before comparing the `.bannerlator_build` marker with the live branch build — JavaSteam's check is unchanged (synced-build hint). `updateNow` / `ensureCurrentBuild` / `verifyFiles` run their delta / verify passes through 2-A (`installApp(..., verify)`); a verify clears `.DepotDownloader/` and, on the Rust engine, `.bl_depot/` too. An "incomplete" verdict from 2-A triggers the existing one-shot verify re-run. | branch p2 |
| 2 — cloud / achievements on the engine | NOT started. The pre-flight's cloud step is SKIPPED when the flag is ON until 2b lands. | — |
| 3 / 4 | NOT started. | — |
