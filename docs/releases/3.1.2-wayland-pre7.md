...
> 🧪 **v9 test layer, added 14 September:** `proton-11.0-2.1-arm64ec-wayland-v9.wcp` (installs as `Proton-11.0-2.1-arm64ec-9`) fixes native OpenGL games that stay **black with sound** on phones that do not let apps reach their display device (seen on retail Adreno 830/840 phones; the session log shows `main device 0:0` and `0 GPU frames from games`). OpenGL now starts on the Vulkan driver directly instead of falling back to a software renderer the layer does not contain. It is being confirmed on those phones now; `v8` stays attached until it is. Only the layer changed: this release's app works with both.
...
| `proton-11.0-2.1-arm64ec-wayland-v9.wcp` | **test layer:** v8 + OpenGL on phones with no reachable display device (black-with-sound fix), installs as **Proton-11.0-2.1-arm64ec-9** |
