...
<!-- update-summary: Wayland pre-release 7 for testers: new containers keep Wayland and their drivers, no blank compositor driver, no Mono prompt, OpenGL safe mode, Control Panel fixed, OpenGL on phones that hide their display device (v9 layer). Not offered in-app. -->
...
> ⚠️ **Read before testing.** Wayland is opt-in per container. It needs the Wayland Proton layer attached to this release (`proton-11.0-2.1-arm64ec-wayland-v9.wcp`); on any other layer the Wayland choice is greyed out and a launch falls back to X11 with a notice. Expect rough edges listed under *What it cannot do yet*.
...
| `proton-11.0-2.1-arm64ec-wayland-v9.wcp` | the Wayland Proton layer (Proton 11.0-2 + winewayland + eight bundled Turnips + xkb data + the desktop-close fix + the Control Panel fix + OpenGL on phones that hide their display device), installs as **Proton-11.0-2.1-arm64ec-9** |
...
2. Install the wcp: **Contents → Proton → Install from file**. It installs as `Proton-11.0-2.1-arm64ec-9`, its own layer line, so it is never offered as an "update" to a normal 11.0-2 container. A container already on `-7` or `-8` shows an **Update layer** button that moves it to `-9` in place (registry backed up first, revertable). Remove the older `-1` to `-6` entries if you have them.
...
4. Container settings: Proton = `Proton-11.0-2.1-arm64ec-9`, **Display backend = Wayland**, Compositor driver = your Android Turnip (filled in for you), Wayland game driver = **Auto**, FEXCore = an installed version. DXVK / VKD3D, Wine components, esync and audio: the same as you use on X11.
